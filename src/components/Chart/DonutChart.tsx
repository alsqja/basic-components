  ...
  useEffect(() => {
    ...
    svg
      ...
      .on("mouseover", function (event) {
        ...
        textTop.text(currentTarget.datum().data.label).attr("y", -10);
        textBottom.text(currentTarget.datum().data.value + "m").attr("y", 10);
      })
      .on("mouseout", function () {
        d3.select(this).transition().duration(100).attr("d", arc);
        textTop.text("TOTAL").attr("y", -10);
        textBottom.text(total + "m");
      })
      .transition()
      .duration(1000)
      .attrTween("d", (d: any) => {
      ...
      <Legend>
        {data.map(({ label, value }, index) => (
          <g key={index}>
            ...
            <text x={45} y={12 + index * 20} fill="black" textAnchor="middle">
              {label}
            </text>
          </g>
        ))}
